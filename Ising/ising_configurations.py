...
# max_T.
Temps = np.linspace(min_T, max_T, num_T) #min_T + np.random.random(num_T)*(max_T - min_T)
...
if training_data:
    np.save("ising_training_labels_%ix%i.npy"%(N,N), train_labels)
    np.save("ising_training_configs_%ix%i.npy"%(N,N), train_configs)
else:
    np.save("ising_test_labels_%ix%i.npy"%(N,N), train_labels)
    np.save("ising_test_configs_%ix%i.npy"%(N,N), train_configs)
